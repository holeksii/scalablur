...
import scalablur.Img
import scalablur.HorizontalBoxBlur
import scalablur.VerticalBoxBlur
...
class PhotoCanvas extends JComponent:
  ...
  private def loadScalaImage(): Img =
    val stream = this.getClass.getResourceAsStream("/scalablur/scala.jpg")
    try
      loadImage(stream)
    finally
      stream.close()
  ...
  def applyFilter(filterName: String, numTasks: Int, radius: Int): Unit =
    ...
    filterName match
      case "horizontal-box-blur" =>
        HorizontalBoxBlur.blur(image, dst, numTasks, radius)
      case "vertical-box-blur" =>
        VerticalBoxBlur.blur(image, dst, numTasks, radius)
      case "" =>
    image = dst
    repaint()
